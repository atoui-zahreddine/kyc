...
export const Applicant = (props: RouteComponentProps<{ url: string }>) => {
  ...
  const totalItems = useAppSelector(state => state.applicant.totalItems);
    ...
    <div>
      ...
      <div className="table-responsive">
        {applicantList && applicantList.length > 0 ? (
            ...
            <tbody>
              {applicantList.map((applicant, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{applicant.createdAt ? <TextFormat type="date" value={applicant.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
          ...
          !loading && <div className="alert alert-warning">No Applicants found</div>
        )}
      </div>
      {totalItems ? (
